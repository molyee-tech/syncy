...
pub mod pallet {
    ...
    pub enum Event<T: Config> {
        ...
        Updated { id: VotingId, voter: T::AccountId, state: State<T::AssetBalance> },
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create(
            ...
            let min_deposit = T::DepositBase::get() + BalanceOf::<T>::from((call_len + 31 / 32) as u32);
            ensure!(deposit >= min_deposit, Error::<T>::LowDepositValue);
        ...
        pub fn vote(
            ...
            let state = Self::put_vote(&voter, &v.asset, id, sign)?;
            Self::deposit_event(Event::<T>::Updated { id, voter, state });
            Ok(())
        }
        ...
        pub fn unvote(
            ...
            let voter = ensure_signed(origin)?;
            let v = Votings::<T>::get(&id).ok_or_else(|| Error::<T>::NotFound)?;
            let state = Self::pop_vote(&voter, &&v.asset, id)?;
            Self::deposit_event(Event::<T>::Updated { id, voter, state });
            Ok(())
        }
        ...
        pub fn execute(
            ...
            ensure!(state.votes == votes, Error::<T>::VotesChanged);
            let total = Self::total(&asset)?;
            let limit = T::RelativeThresholdLimit::get();
            ensure!(state.is_reached(v.threshold, total, limit), Error::<T>::ThresholdIsNotReached);
            let accounts: Vec<_> = Votes::<T>::iter_prefix(&id).map(|p| p.0).collect();
            for u in accounts.iter() {
                let _ = Self::remove_vote(u, &asset, id)?;
            }
            let res = Self::execute_call(id, v, max_weight)?.actual_weight;
            Ok(res.map(|w| T::WeightInfo::execute(votes).saturating_add(w)).into())
        }
...
impl<T: Config> Pallet<T> {
    ...
    fn put_vote(
        ...
        let key = (voter.clone(), asset.clone());
        let value =
